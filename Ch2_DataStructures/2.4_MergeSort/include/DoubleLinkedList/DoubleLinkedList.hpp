...
protected:
    ...
    size_type get_count(std::pair<node*,node*>) noexcept;
    node* get_middle(node*, size_type) noexcept;
    std::pair<node*,node*> do_merge(std::pair<node*,node*>,
                                    std::pair<node*,node*>) noexcept;
    std::pair<node*,node*> do_sort_merge(std::pair<node*,node*>) noexcept;
